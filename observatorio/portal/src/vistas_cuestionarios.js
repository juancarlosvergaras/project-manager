...
<div class="mintic-page-intro mb-4"><div><h1>Cuestionarios</h1><p>Instrumentos del Observatorio: se diseñan aquí, se aplican por enlace público o por campañas de correo, y sus respuestas alimentan el cuadro de mando.</p></div>
...
<div class="row g-4 mt-3">
  <div class="col-lg-4"><div class="card h-100"><div class="card-body"><h2 class="h6 text-primary">Cómo se aplica un cuestionario</h2><p class="small mb-0">Cada cuestionario publicado tiene un enlace público que puede compartirse por cualquier medio. Con una campaña se envía además un enlace personal a cada correo de una lista, se programa la fecha de envío y se recuerda a quienes no han respondido.</p></div></div></div>
  <div class="col-lg-4"><div class="card h-100"><div class="card-body"><h2 class="h6 text-primary">Los ejemplos del proyecto</h2><p class="small mb-0">Los tres instrumentos ya aplicados (Información No Verificada, Autodiagnóstico Integrado e Infraestructura Computacional) están cargados tal cual, con su estilo, sus políticas de datos y sus reglas de cálculo. Están protegidos: para adaptarlos, duplíquelos.</p></div></div></div>
  <div class="col-lg-4"><div class="card h-100"><div class="card-body"><h2 class="h6 text-primary">Resultados y cuadro de mando</h2><p class="small mb-0">Cada cuestionario tiene su panel de resultados con cifras, distribuciones, promedios por dimensión, filtros y exportación a hoja de cálculo. Los promedios por ámbito del autodiagnóstico alimentan el diagnóstico de preparación del cuadro de mando del Observatorio.</p></div></div></div>
</div>
...
<div class="mintic-page-intro mb-4"><div><h1>Importar respuestas anteriores</h1><p><strong>${esc(d.titulo)}</strong>. Suba la exportación a Excel (.xlsx) o CSV del formulario anterior: el portal lee las columnas, propone a qué pregunta corresponde cada una y usted confirma antes de importar. Las respuestas quedan dentro de una campaña, con su fecha original, y entran al panel de resultados y al cuadro de mando.</p>
...
<div class="mintic-page-intro-note"><strong>Cómo se interpretan los valores</strong>Las opciones se emparejan por texto (sin distinguir mayúsculas ni tildes); las respuestas múltiples se separan por punto y coma; en escalas y rúbricas vale el número inicial («3 - Definido»); Sí/No acepta sí, no, 1 y 0. Lo que no coincida se guarda tal cual.</div></div>
...
<div class="card"><div class="card-header">Nueva campaña</div><div class="card-body"><form method="post" action="/cuestionarios/${cuestionario.id}/campanias" data-working-text="Creando...">
  <div class="row g-3">
    ...
    <div class="col-md-6"><label class="form-label">Mensaje</label><textarea class="form-control" name="cuerpo" rows="12" required>${esc(CUERPO_POR_DEFECTO.replace(/\{\{\s*cuestionario\s*\}\}/g, d.titulo))}</textarea><div class="form-text">El nombre del cuestionario ya está puesto. Lo que cambia por persona queda como variable y se rellena al enviar: <code>{{nombre}}</code>, <code>{{entidad}}</code>, <code>{{correo}}</code> y <code>{{enlace}}</code> (el enlace personal, que se convierte en botón). Admite **negritas**.</div></div>
...
export function vistaCampania({ usuario, apps, ids, cuestionario, campania: k, destinatarios, mensaje = '', error = '', previa, smtpOk, enviando }) {
  ...
  const filas = destinatarios.map((x, i) => `<tr><td>${i + 1}</td><td>${esc(x.correo)}<div class="small text-muted">${esc(x.nombre || '')}${x.entidad ? ' · ' + esc(x.entidad) : ''}</div></td><td>${badgeDestinatario(x.estado)}${x.error ? `<div class="small text-danger">${esc(x.error)}</div>` : ''}</td><td class="small text-nowrap">${x.enviado_en ? fechaCo(x.enviado_en) + (x.envios > 1 ? ` <span class="text-muted">(${x.envios} envíos)</span>` : '') : ''}</td><td class="small text-nowrap">${x.respondido_en ? fechaCo(x.respondido_en) : ''}</td><td class="text-nowrap"><button class="btn btn-sm btn-outline-secondary py-0" type="button" data-copiar="${esc(enlaceDe(cuestionario.clave, x.token))}">Copiar enlace</button> <a class="btn btn-sm btn-outline-secondary py-0" href="/campanias/${k.id}/destinatarios/${x.id}/qr.svg?descargar=1">QR</a></td><td>${x.estado === 'respondido' ? '' : `<form method="post" action="/campanias/${k.id}/destinatarios/${x.id}/quitar" class="d-inline"><button class="btn btn-sm btn-outline-danger py-0">Quitar</button></form>`}</td></tr>`).join('');
...
<div class="mintic-page-intro-note"><strong>Enlace público del cuestionario</strong><code class="small">${esc(config.urlPublica)}/c/${esc(cuestionario.clave)}</code><div class="d-flex align-items-center gap-2 mt-2"><img class="qr-mini" src="/c/${esc(cuestionario.clave)}/qr.svg?pie=0" alt="QR del enlace público"><a class="small" href="/c/${esc(cuestionario.clave)}/qr.svg?descargar=1">Descargar QR</a></div></div></div>
${mensaje ? `<div class="alert alert-success">${esc(mensaje)}</div>` : ''}${error ? `<div class="alert alert-danger">${esc(error)}</div>` : ''}

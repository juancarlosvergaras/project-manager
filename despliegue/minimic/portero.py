#!/usr/bin/env python3
"""Portero de minimic.proyectoia.org: pasa al PC que tenga el teclado.

El teclado de voz se lleva de un PC a otro, y en cada uno corre su propio
MiniMic. La direccion web es una sola, asi que el portero pregunta a cada PC
...
ESCUCHA = ('127.0.0.1', int(os.environ.get('PORTERO_PUERTO', '8025')))
...
DESTINOS = [
    (h.strip(), int(p)) for h, p in (
        d.split(':') for d in os.environ.get(
            'PORTERO_PCS', '100.79.52.120:8771,100.125.175.45:8771,100.66.117.114:8771'
        ).split(',')
    )
]
PLAZO_S = 3.0
...
<title>MiniMic - ningun equipo del teclado esta encendido</title>
<style>
...
<h1>Ningun equipo con MiniMic esta encendido</h1>
<p>Esta pagina esta bien; el servidor tambien. Lo que falta es un ordenador que
tenga MiniMic en marcha y publicado por Tailscale.</p>
...
<p>Enciende ese equipo, o en su panel local (http://127.0.0.1:8771, pestana
...
async def salud(destino):
    ...
    try:
        ...
        return None
    if not isinstance(datos, dict) or datos.get('app') != 'minimic':
        return None
...
async def principal():
    ...
    print(f'Portero de minimic en {ESCUCHA[0]}:{ESCUCHA[1]} -> {DESTINOS}', flush=True)
    async with servidor:

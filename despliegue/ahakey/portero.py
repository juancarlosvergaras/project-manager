#!/usr/bin/env python3
"""Portero de ahakey.proyectoia.org: pasa al PC que tenga el AhaKey.

Dos maneras de encontrar ese PC, y la primera es la buena:

1. **El PC se presenta solo.** El servicio TecladoIA de cada equipo abre por
   Tailscale una conexión de control a este portero (puerto 8030, solo en la
   ...
   Antes el portero iba a buscar una dirección fija, y bastaba que Tailscale
   arrancara después que el servicio para que la web dijera «el equipo está
   apagado» con el equipo encendido (15/9/2026).
2. **Si ningún PC se ha presentado**, se pregunta a una lista fija de
   direcciones por Tailscale (`/api/salud`, sin clave), como antes.

Si no hay nadie, una página que explica que el equipo está apagado, en vez
del «Bad gateway» de Cloudflare. Proxy de nivel TCP: solo se interpreta HTTP
para la pregunta de salud; el resto se empalma tal cual, así que las
...
APP = os.environ.get('PORTERO_APP', 'tecladoia')
ESCUCHA = ('127.0.0.1', int(os.environ.get('PORTERO_PUERTO', '8024')))
...
ESCUCHA_AGENTES = (
    ...
    int(os.environ.get('PORTERO_AGENTES_PUERTO', '8030')),
)
#: Los PC candidatos del camino viejo, en orden de preferencia.
DESTINOS = [
    (h.strip(), int(p)) for h, p in (
        d.split(':') for d in os.environ.get(
            'PORTERO_PCS', '100.79.52.120:8770,100.125.175.45:8770,100.66.117.114:8770'
        ).split(',')
    )
]
PLAZO_S = 3.0
...
<title>TecladoIA - el equipo del teclado esta apagado</title>
<style>
 ...
 .pie{margin-top:1.5rem;padding-top:1rem;border-top:1px solid var(--borde);font-size:.9rem}
</style></head><body><div class="caja">
<h1>Ningun equipo con el AhaKey esta encendido</h1>
<p>Esta pagina esta bien; el servidor tambien. Lo que falta es un ordenador que
tenga TecladoIA en marcha y conectado a Tailscale.</p>
<p><b>El Bluetooth no viaja por Internet</b>: la aplicacion corre en el equipo que
tiene el teclado emparejado, y ese equipo se presenta aqui solo, sin
configurar nada. Esta direccion pasa al que tenga el teclado en ese momento.</p>
<p>Enciende ese equipo y vuelve a cargar. El servicio arranca solo al iniciar
sesion y se revisa cada diez minutos por si acaso.</p>
<p class="pie">Si el ordenador esta encendido y aun asi lees esto: comprueba que
Tailscale esta conectado y que TecladoIA tiene clave (su panel local,
http://127.0.0.1:8770); sin clave no se publica. Para arrancar el servicio a
mano: <code>schtasks /Run /TN TecladoIA</code></p>
</div></body></html>'''

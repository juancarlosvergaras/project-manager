#!/usr/bin/env python3
"""Portero de ledblanco.proyectoia.org (la Botonera): pasa al PC que tenga el teclado.

Dos maneras de encontrar ese PC, y la primera es la buena:

1. **El PC se presenta solo.** El servicio Botonera de cada equipo abre por
   Tailscale una conexión de control a este portero (puerto 8029, solo en la
...
APP = os.environ.get('PORTERO_APP', 'botonera')
ESCUCHA = ('127.0.0.1', int(os.environ.get('PORTERO_PUERTO', '8028')))
...
<title>Botonera - ningun equipo del teclado esta encendido</title>
<style>
...
<h1>Ningun equipo con la Botonera esta encendido</h1>
<p>Esta pagina esta bien; el servidor tambien. Lo que falta es un ordenador que
tenga la Botonera en marcha y conectado a Tailscale.</p>
...
<p>Enciende ese equipo, comprueba que Tailscale esta conectado y que la Botonera
tiene clave (pestana Ajustes de su panel, http://127.0.0.1:8773): sin clave no
se publica.</p>
</div></body></html>'''

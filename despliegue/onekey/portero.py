#!/usr/bin/env python3
"""Portero de onekey.proyectoia.org: pasa al PC que tenga el botón AI_VOICE.

Dos maneras de encontrar ese PC, y la primera es la buena:

1. **El PC se presenta solo.** El servicio OneKey de cada equipo abre por
   Tailscale una conexión de control a este portero (puerto 8033, solo en la
...
APP = os.environ.get('PORTERO_APP', 'onekey')
ESCUCHA = ('127.0.0.1', int(os.environ.get('PORTERO_PUERTO', '8032')))
...
ESCUCHA_AGENTES = (
    ...
    int(os.environ.get('PORTERO_AGENTES_PUERTO', '8033')),
)
#: Los PC candidatos del camino viejo, en orden de preferencia.
DESTINOS = [
    (h.strip(), int(p)) for h, p in (
        d.split(':') for d in os.environ.get(
            'PORTERO_PCS', '100.79.52.120:8774,100.125.175.45:8774,100.66.117.114:8774'
        ).split(',')
    )
]
PLAZO_S = 3.0
...
<title>OneKey - el equipo del boton esta apagado</title>
<style>
...
<h1>Ningun equipo con el boton AI_VOICE esta encendido</h1>
<p>Esta pagina esta bien; el servidor tambien. Lo que falta es un ordenador que
tenga OneKey en marcha y conectado a Tailscale.</p>
<p><b>El Bluetooth no viaja por Internet</b>: la aplicacion corre en el equipo que
tiene el boton emparejado, y ese equipo se presenta aqui solo, sin
configurar nada. Esta direccion pasa al que tenga el boton en ese momento.</p>
...
Tailscale esta conectado y que OneKey tiene clave (su panel local,
http://127.0.0.1:8774); sin clave no se publica. Para arrancar el servicio a
mano: <code>schtasks /Run /TN OneKey</code></p>
</div></body></html>'''

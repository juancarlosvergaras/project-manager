"""El panel web de OneKey: HTTP a mano sobre asyncio, como en los demás.

Mismas reglas de la casa: fuera de la máquina local exige clave (cabecera
``X-OneKey-Clave``, ``Authorization: Bearer``, cookie ``onekey`` o ``?clave=``
una sola vez, que se convierte en cookie); comparación de tiempo constante;
estáticos sin salir de su carpeta; y un canal de sucesos (SSE) con latido cada
veinte segundos para que ningún intermediario corte la conexión.
"""
...
_CAMPOS_AJUSTES = {
    ...
    "usar_portero": bool, "portero": str, "boton_bluetooth": str,
}
...
class PanelWeb:
    ...
    async def _atender(self, lector: asyncio.StreamReader, escritor: asyncio.StreamWriter) -> None:
        try:
            ...
            if partes.path == "/descargar/" + empaquetado.NOMBRE_EXE and metodo == "GET":
                ...
                # para no abrir dos servicios, y lo que mira el portero.
                estado, tipo, datos = self._json_ok({
                    "app": "onekey", "version": __version__,
                    "teclado": self.servicio.estado.boton is not None,
                ...
                extras.append(f"Set-Cookie: onekey={self.ajustes.clave_panel}; Path=/; HttpOnly; SameSite=Strict")
    ...
    def _autorizado(self, cabeceras: dict[str, str], consulta: dict[str, list[str]], es_local: bool = False) -> bool:
        ...
        if cabecera := cabeceras.get("x-onekey-clave"):
    ...
    def _descarga(self) -> tuple[str, str, bytes]:
        ...
        if ruta is None:
            return self._error("404 Not Found", "el instalador no está construido en este equipo")
    ...
    async def _api(self, metodo: str, ruta: str, datos: dict[str, Any]) -> tuple[str, str, bytes]:
        ...
        if ruta == "/api/boton/buscar" and metodo == "POST":
            return self._json_ok(await en_hilo(s.buscar_boton))
        ...
        if ruta == "/api/dictado/probar" and metodo == "POST":
            return self._json_ok(await en_hilo(s.al_pulsar_microfono, "prueba desde el panel"))

"""El panel web de SikaiMini: HTTP a mano sobre asyncio, como en TecladoIA.

Mismas reglas de la casa: fuera de la máquina local exige clave (cabecera
``X-SikaiMini-Clave``, ``Authorization: Bearer``, cookie ``sikaimini`` o
...
class PanelWeb:
        ...
        try:
            ...
            if partes.path == "/descargar/" + empaquetado.NOMBRE_EXE and metodo == "GET":
                ...
                estado, tipo, datos = self._json_ok({
                    "app": "sikaimini", "version": __version__,
    ...
    def _autorizado(self, cabeceras: dict[str, str], consulta: dict[str, list[str]], es_local: bool = False) -> bool:
        ...
        if cabecera := cabeceras.get("x-sikaimini-clave"):
    ...
    async def _api(self, metodo: str, ruta: str, datos: dict[str, Any]) -> tuple[str, str, bytes]:
        ...
        if ruta == "/api/opciones" and metodo == "GET":
            return self._json_ok({
                ...
                "multimedia": list(protocolo.CONSUMO),
        ...
        if ruta == "/api/teclas" and metodo == "POST":
            ...
            if not isinstance(teclas, list) or not all(isinstance(t, str) for t in teclas):
                raise ValueError("«teclas» tiene que ser una lista de seis textos: tres teclas y tres gestos de la perilla")
        ...
        if ruta == "/api/teclas/recomendado" and metodo == "POST":
            return self._json_ok(await en_hilo(s.volver_a_lo_recomendado))
        if ruta == "/api/luces" and metodo == "POST":
            modo, color = datos.get("modo"), datos.get("color")
            if not isinstance(modo, int) or isinstance(modo, bool):
                raise ValueError("falta «modo» (número; -1 para no tocar las luces)")
            if not isinstance(color, str):
                raise ValueError("falta «color» (#RRGGBB)")
            return self._json_ok(await en_hilo(s.poner_luces, modo, color))
        if ruta == "/api/luces/leer" and metodo == "POST":
            return self._json_ok(await en_hilo(s.leer_luces))

"""Órdenes de SikaiMini: ``servicio``, ``estado``, ``teclas``, ``tarea``."""
...
def hay_otro_servicio(ajustes: Ajustes) -> Optional[dict]:
    """Pregunta en el puerto del panel si ya hay un SikaiMini vivo."""
...
def orden_servicio(args: argparse.Namespace) -> int:
    ...
    async def ejecutar() -> int:
        ...
        logging.getLogger("sikaimini").info(
...
def construir_analizador() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="sikaimini", description=f"{NOMBRE} {__version__}: el mini teclado SiKai de tres teclas y perilla, en español.")
    ...
    t = sub.add_parser("teclas", help="lee las teclas, la perilla y las luces del teclado (por cable)")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    logging.basicConfig(level=os.environ.get("SIKAIMINI_NIVEL") and logging.DEBUG or nivel, format=_FORMATO, datefmt="%H:%M:%S")
    try:
